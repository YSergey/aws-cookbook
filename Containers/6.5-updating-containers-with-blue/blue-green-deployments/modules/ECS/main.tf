...
resource "aws_ecs_task_definition" "green" {
  ...
  container_definitions = jsonencode([{
    name      = "my-green-container"
    image     = "129972428809.dkr.ecr.us-west-2.amazonaws.com/updating-containers-with-blue-green-deployments-container-repo:green"
    cpu       = 256
    memory    = 512
    essential = true
    portMappings = [
...
Resources:
  - TargetService:
      ...
      Properties:
        TaskDefinition: "${aws_ecs_task_definition.green.arn}"
        LoadBalancerInfo:
          ContainerName: "my-green-container"
          ContainerPort: 80
EOF
}
